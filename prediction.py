...
def visualize_probs(vocab, probs, col_width=4, col_height=300):
    ...
    top = np.argmax(probs)
    cv2.putText(out, f'P: {vocab[top]}', (100, 100), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 0, 0), 1)
    cv2.putText(out, f'C: {probs[top]:.3f}', (200, 200), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 255, 0), 1)
    ...
    print(f'Prediction: {vocab[top]}, Confidence: {probs[top]:.3f}')
    return out


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--config-path', type=Path, default=Path('C:\\Users\\naimu\\Desktop\\TCN-csdntest\\configs\\lrw_resnet18_mstcn.json'))
    parser.add_argument('--model-path', type=Path, default=Path('C:\\Users\\naimu\\Desktop\\TCN-csdntest\\models\\lrw_resnet18_mstcn.pth.tar'))
    # parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument('--device', type=str, default='cpu')
    parser.add_argument('--queue-length', type=int, default=25)
    ...
    mean_face_landmarks = np.load(Path('C:\\Users\\naimu\\Desktop\\TCN-csdntest\\preprocessing\\20words_mean_face.npy'))

    with Path('C:\\Users\\naimu\\Desktop\\TCN-csdntest\\labels\\500WordsSortedList.txt').open() as fp:
        vocab = fp.readlines()
    assert len(vocab) == 500

    queue = deque(maxlen=args.queue_length)

    with VideoCapture("C:\\Users\\naimu\\Desktop\\TCN-csdntest\\ABOUT_00007.mp4") as cap:
        print('I am in line 93')
        while True:
            ret, image_np = cap.read()
            print('I am in line 96')
            if not ret:
                break
            image_np = cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB)

            all_landmarks = fa.get_landmarks(image_np)
            print('I am in line 102')
            if all_landmarks:
